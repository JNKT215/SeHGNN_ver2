...
def get_final_score(cfg,data,best_pred,full_loader,model,checkpt_file,labels,device):
    ...
    if cfg["dataset"] != 'IMDB':
        ...
        data.dl.gen_file_for_macnemar(test_idx=data.test_nid, label=pred,true_label=labels[data.test_nid],file_name=f"{cfg.dataset}_{cfg.seeds}_macnemar_{checkpt_file.split('/')[-1]}.txt")
        ...
        data.dl.gen_file_for_macnemar(test_idx=data.test_nid, label=pred,true_label=labels[data.test_nid],file_name=f"{cfg.dataset}_{cfg.seeds}_macnemar_{checkpt_file.split('/')[-1]}.txt", mode='multi')
