...
def get_metapath_l(metapath_name,node_types,edge_types,reverse=False):
    metapath_l, metapath_dict,cnt = [],{},0
    edge_types_dict = {edge_type_tuple[1]:edge_type_tuple for edge_type_tuple in edge_types}
    for node_type in node_types:
        for metapath in metapath_name[node_type]:
            if len(metapath) == 2:
                if not reverse:
                    metapath_dict[metapath] = metapath
                else:
                    metapath_dict[metapath[::-1]] = metapath
                continue
            if not reverse:
                metapath_split = [metapath[i:i+2] for i in range(len(metapath)-1)]
            else:
                metapath_split = [metapath[i:i+2][::-1] for i in reversed(range(len(metapath)-1))]
            metapath_tuple = [edge_types_dict[edge_type]for edge_type in reversed(metapath_split)]
            metapath_l.append(metapath_tuple)
            metapath_dict[f'metapath_{cnt}'] = metapath
            cnt+=1
    return metapath_l,metapath_dict



def get_metapath_l_only_tgt_type(metapath_name,tgt_node_type,edge_types,reverse=False):
    metapath_l, metapath_dict,cnt = [],{},0
    edge_types_dict = {edge_type_tuple[1]:edge_type_tuple for edge_type_tuple in edge_types}
    for metapath in metapath_name[tgt_node_type]:
        if len(metapath) == 2:
            if not reverse:
                metapath_dict[metapath] = metapath
            else:
        ...
        if not reverse:
            ...
            metapath_split = [metapath[i:i+2][::-1] for i in reversed(range(len(metapath)-1))]
        metapath_tuple = [edge_types_dict[edge_type]for edge_type in metapath_split]
        metapath_l.append(metapath_tuple)
        metapath_dict[f'metapath_{cnt}'] = metapath
        cnt+=1
    return metapath_l,metapath_dict
...
def enum_metapath_name(name_dict, type_dict, length):
    # 考えられるすべてのメタパス名を列挙します
    # 予想される全てのmetapath 名を列挙する
    # 結果はタイプ別に返される
    #name_dict → エッジタイプ：例 {0: 'MD', 1: 'DM', 2: 'MA', 3: 'AM'}
    #type_dict →次のノードタイプ：例 {0: [1], 1: [0, 2], 2: [3], 3: [0, 2]}
    #length → metapath の長さ
    hop = []
    path_list = []
    result_dict = {}
    #それぞれのノードタイプを先頭としたときに考えられる metapathを格納するための，辞書型を作成
    #例) {'M': [], 'D': [], 'A': []}
    for type in type_dict.keys():
        hop.append([type])
        result_dict[name_dict[type][0]] = []
    path_list.extend(hop)
     #それぞれのノードタイプを先頭としたときに考えられる metapathの列挙
    for i in range(length - 2):
        new_hop = []
        for path in hop:
            for next_type in type_dict[path[-1]]:
...
def combine_features_dict(list_of_node_dict, batch_src_index, batch_src_label, DEVICE):
    ...
    # まず、ポイントの種類の数をカウントし、ポイント番号をカテゴリに分類して辞書に格納します。
    ...
    for type in type_dict:
        # 初始化每类的特征、张量和行号记录字典
        # 各クラスの特徴、テンソル、行番号レコードの辞書を初期化します。
    ...
    for type in type_dict:
        for metapath in metapath_dict[type]:
            ...
            for node_index in type_dict[type]:
                ...
                # 各ポイントの各メタパスのフィーチャ行の数をカウントし、フィーチャ行の数を記録します。
                if metapath not in list_of_node_dict[node_index]:
                    # 该点没有此metapath，记录0
                    # このポイントにはこのメタパスがありません。レコード 0
